...
function AddServiceDialog({ open, onOpenChange, onAddServices }: AddServiceDialogProps) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Select the services you want to add to your business.
          </DialogDescription>
        </DialogHeader>
        
        <div className="grid gap-4 py-4">
          {hardcodedServices.map((serviceName, index) => (
...
export function ServicesTable() {
  ...
  // Add handler for adding services
  const handleAddServices = async (selectedServices: string[]) => {
    try {
      const url = getBaseUrlByCategory();
      // Here you would typically make an API call to add the selected services
      // For now, we'll just show a success notification
      showNotification("Success", "Services added successfully", "success");
    } catch (error) {
      showNotification("Error", "Failed to add services: " + error.message, "error");
    }
  };
